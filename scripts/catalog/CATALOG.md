...
- `covers.ts`: downloads covers as `<anilistId>.<ext>`.
...
| `npm run catalog:covers` | Downloads missing covers (optional) |
...
- AniList's terms prohibit hoarding its data. Keep requests to the anime in the library and the fields the game uses, and don't run `catalog:covers` until the owner has decided on covers ([SECURITY.md](../../docs/SECURITY.md#external-services)).
